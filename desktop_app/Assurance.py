...
if getattr(sys, 'frozen', False):
    ...
    graphviz_base_path = os.path.join(sys._MEIPASS, 'graphviz')
    # Set environment variables for Graphviz
    os.environ['GRAPHVIZ_DOT'] = os.path.join(graphviz_base_path, 'bin', 'dot')
    os.environ['GVPLUGINPATH'] = os.path.join(graphviz_base_path, 'lib', 'graphviz_plugins')
# else:
#     # Normal execution
    ...
    # Page that indicates training is occurring
        ...
        if training_complete == 0:
            ...
            try:
                ...
                return render_template('trueoutput.html', old_exists=old_exists)
...
def download_csv():
    ...
    try:
        ...
        return render_template('datarep.html', data_subset=data_subset, length=result.shape[0], training_complete=training_complete)
    except Exception as e:
        return jsonify({"error": str(e)})
...
def dataset1report():
    ...
    print(data_subset)
    return render_template('datarep.html', data_subset=data_subset, length=result.shape[0], training_complete=0)
