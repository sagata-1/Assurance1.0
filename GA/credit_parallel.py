...
oversampled_fraud = pd.concat([oversampled_fraud, inputData[inputData["Class"] == 1]], ignore_index=True)
...
def main():
    ...
    # Read in the data
    dict_conversion = {0: 1, 1: 2}
    reverse_conversion = {1: 0, 2: 1}
    ...
    val = deterministic_ga(chromosome_length, 100, 100, eng_vec, number_of_runs, feature_data, response_categories, max_feature_value, min_feature_value, number_of_leaves, number_of_nodes, number_of_classes, number_of_features)
    print(val[2])
    print(f"{val[0]:.2f}")
    end = time.time()
    print(f"Running time: {end - start}")
    dot_data = array_to_dot(val[2], inputData)
    # print(dot_data)
    ...
    predictions = []
    for i in range(number_of_runs):
        predictions.append(reverse_conversion[int(tree_model_predict(chromosome_length,feature_data[i, :], val[2], prediction_category))])
    # print(predictions)
    inputData["Predictions"] = predictions
...
#     # inacc_num, imprecision, not-fraud, insensitivity, fraud
#     # print(prediction_categor, response_category)
#     if int(response_category) == 1:
#         if int(prediction_categor) != int(response_category):
#             return (1, 1, 1, 0, 0)
#         return (0, 0, 1, 0, 0)
#     elif int(response_category) == 2:
#         if int(prediction_categor) != int(response_category):
#             return (1, 0, 0, 1, 1)
#         return (0, 0, 0, 0, 1)
...
#     return (sum(errors[0]) / len(response_categories) + 0.5* (((sum(errors[1]) / sum(errors[2])) + (sum(errors[3]) / sum(errors[4]))) / 2), 0.5* (((sum(errors[1]) / sum(errors[2])) + (sum(errors[3]) / sum(errors[4]))) / 2), (sum(errors[1]) / sum(errors[2])), (sum(errors[3]) / sum(errors[3])))
...
def estimate_prediction_error(chromosome_length, person_tree, number_of_runs, data, response_categories):
...
def class_tree_translate_to_engineering(number_decision_variables, x_vector, engineering_x_vector, max_feature_value, min_feature_value, number_of_leaves, number_of_nodes, number_of_classes, number_of_features):
...
def a4_function(number_decision_variables, x_vector, class_tree_translate_to_engineering, estimate_prediction_error, engineering_x_vector, number_of_runs, data, response_categories, max_feature_value, min_feature_value, number_of_leaves, number_of_nodes, number_of_classes, number_of_features):
    # Part 1: Interpret the [0,1] hypercube vector as a solution.
    engineering_loc = class_tree_translate_to_engineering(number_decision_variables, x_vector, engineering_x_vector, max_feature_value, min_feature_value, number_of_leaves, number_of_nodes, number_of_classes, number_of_features)

    # Part 2: Evaluate the solution.- fitness valuation
    error_value = estimate_prediction_error(number_decision_variables, engineering_loc, number_of_runs, data, response_categories)

    return error_value

def a4_translate_to_engineering(number_decision_variables, x_vector, class_tree_translate_to_engineering, engineering_x_vector, max_feature_value, min_feature_value, number_of_leaves, number_of_nodes, number_of_classes, number_of_features):
    engineering_loc = class_tree_translate_to_engineering(number_decision_variables, x_vector, engineering_x_vector, max_feature_value, min_feature_value, number_of_leaves, number_of_nodes, number_of_classes, number_of_features)
    return engineering_loc
...
def deterministic_ga(number_decision_variables, number_in_population, number_of_generations, engineering_x_vector, number_of_runs, data, response_categories, max_feature_value, min_feature_value, number_of_leaves, number_of_nodes, number_of_classes, number_of_features):
    # Legacy
    pool = mp.Pool(2)
    ...
    for g_index in range(number_of_generations):
        ...
        #     temp = a4_function(1, number_decision_variables, x_vector, class_tree_translate_to_engineering, estimate_prediction_error, engineering_x_vector)
        #     print(temp)
        ...
        args_list = [(number_decision_variables, current_generation[i_index, :], class_tree_translate_to_engineering, estimate_prediction_error, engineering_x_vector, number_of_runs, data, response_categories, max_feature_value, min_feature_value, number_of_leaves, number_of_nodes, number_of_classes, number_of_features) for i_index in range(number_in_population)]
        results = pool.starmap(a4_function, args_list)
        # print(results)
        for i_index, (objective_value, norm_value, imprec, insens) in enumerate(results):
            current_objective_values[i_index] = objective_value
            normalizer[i_index] = norm_value
            proportions[i_index] = (imprec, insens)
        # Sort the population
        sort_index = np.argsort(current_objective_values)
        current_generation = current_generation[sort_index, :]
        current_objective_values = current_objective_values[sort_index]
        ...
        for i_index in range(e_elitist + m_immigrant, number_in_population):
            ...
            temp = a4_function(number_decision_variables, first_child, class_tree_translate_to_engineering, estimate_prediction_error, engineering_x_vector, number_of_runs, data, response_categories, max_feature_value, min_feature_value, number_of_leaves, number_of_nodes, number_of_classes, number_of_features)
            temp_1 = a4_function(number_decision_variables, second_child, class_tree_translate_to_engineering, estimate_prediction_error, engineering_x_vector, number_of_runs, data, response_categories, max_feature_value, min_feature_value, number_of_leaves, number_of_nodes, number_of_classes, number_of_features)
    ...
    engineering_loc = a4_translate_to_engineering(number_decision_variables, x_vector, class_tree_translate_to_engineering, engineering_x_vector, max_feature_value, min_feature_value, number_of_leaves, number_of_nodes, number_of_classes, number_of_features)
    return (current_objective_values[0] - normalizer[0], proportions[0], engineering_loc)
