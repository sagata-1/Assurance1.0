...
inputData = pd.read_csv("ion.csv")
testData = pd.read_csv("ion.csv").iloc[176:, :]
oversampled_fraud = inputData[inputData["Class"] == "bad"]
for _ in range(1):
    oversampled_fraud = pd.concat([oversampled_fraud, oversampled_fraud], ignore_index=True)
oversampled_fraud = pd.concat([oversampled_fraud, inputData[inputData["Class"] == "bad"]], ignore_index=True)
...
def main():
    ...
    # Read in the data
    dict_conversion = {"good": 1, "bad": 2}
    reverse_conversion = {1: "good", 2: "bad"}
    ...
    val = deterministic_ga(chromosome_length, 80, 50, eng_vec)
    print(eng_vec)
    print(f"{val[0]:.2f}")
    end = time.time()
