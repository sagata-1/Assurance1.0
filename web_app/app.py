...
def dataset():
    ...
    if request.method == 'GET':
        ...
        flash("Finding Optimal Params, approximately 85 minutes left")
        ...
        print(len(names))
        flash("Training model, approximately 10 minutes left")
        # Train model
        selection_model.fit(select_X_train, y_train)
        # Make prediction
        # selection = SelectFromModel(model, threshold=true_thresh, prefit=True)
...
@app.route("/dataset1report", methods=['GET'])
def dataset1report():
    return render_template('datasetreport.html')
